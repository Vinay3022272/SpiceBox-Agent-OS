"""
create_pages — LLM NODE

Creates new wiki pages for products that don't exist yet.
...
try:
    ...
    from src.agents.store_manager_llm.utils.file_io import write_markdown, get_page_path, slugify, page_exists
    from src.agents.store_manager_llm.utils.llm import call_llm, call_llm_json
...
def _generate_product_page(product: dict, review_synthesis: dict | None, section: str) -> str:
    """Generate a product wiki page using template + LLM enrichment."""
    ...
    context = {
        ...
        "conflicts": [],
        "related_links": [],
        "sources": [product.get("source_file", "unknown")],
    ...
    # If description is sparse, use LLM to generate a richer one
    if len(context["description"]) < 50:
        specs_text = "\n".join(f"- {k}: {v}" for k, v in context["specifications"].items())
        prompt = (
            f"Write a concise 2-3 sentence product overview for '{product['name']}' "
            f"by {product.get('brand', 'unknown brand')}.\n\n"
...
def _generate_marketing_page(product: dict, page_type: str) -> str:
    """Generate a marketing page (promotion/specialty/popular)."""
    ...
    # Use LLM to enrich marketing content
    ...
    try:
        ...
        marketing = {
            "why_promote": f"{product['name']} is a strong product in its category.",
            "selling_points": ["Quality product", "Competitive pricing"],
            "target_audience": ["General consumers"],
            "revenue_note": "Standard revenue potential.",
            "popularity_reason": "Solid product with good reviews.",
        }
    ...
    context = {
        ...
        "category_slug": slugify(product.get("category", "general")),
        "price": product.get("price", "N/A"),
        "currency": product.get("currency", "INR"),
        "rating": "N/A",
        "review_count": 0,
...
def create_pages(state: WikiState) -> dict:
    ...
    for page in pages_to_create:
        ...
        # Generate page content based on section
        if section == "knowledge":
            review_data = review_map.get(slug)
            content = _generate_product_page(product, review_data, section)
        else:
            content = _generate_marketing_page(product, page["page_type"])
        ...
        print(f" Created: {section}/{page['page_type']}/{slug}.md")

    # Also create category pages for knowledge section
    if section == "knowledge":
        categories = set()
        ...
        for cat_name in categories:
            ...
            if not page_exists(wiki_base, section, "categories", cat_slug):
                # Gather products in this category
                ...
                context = {
                    ...
                    "products": [{
                        ...
                        "rating": review_map.get(p["slug"], {}).get("avg_rating", "N/A"),
                        "review_count": review_map.get(p["slug"], {}).get("total_reviews", 0),
                    } for p in cat_products],
                    "top_rated": [],
                    "price_range": None,
                    "currency": "INR",
                    "trends": f"Category contains {len(cat_products)} products.",
                    "sources": list(set(p.get("source_file", "") for p in cat_products)),
                    "last_updated": datetime.now().strftime("%Y-%m-%d"),
                }
                ...
                print(f" Created category: {section}/categories/{cat_slug}.md")

    print(f"  Created {len(generated)} new pages")
