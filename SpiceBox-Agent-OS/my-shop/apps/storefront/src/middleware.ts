...
async function getRegionMap(cacheId: string) {
  ...
  if (
    ...
    regionMapUpdated < Date.now() - 30 * 1000
  ) {
...
export async function middleware(request: NextRequest) {
  ...
  const isStaleCountry = firstSegment && firstSegment.length === 2 && !regionMap.has(firstSegment)
  const cleanPath = isStaleCountry
    ? (segments.length > 1 ? "/" + segments.slice(1).join("/") : "")
    : (request.nextUrl.pathname === "/" ? "" : request.nextUrl.pathname)

  const queryString = request.nextUrl.search || ""
  const redirectUrl = `${request.nextUrl.origin}/${country}${cleanPath}${queryString}`
