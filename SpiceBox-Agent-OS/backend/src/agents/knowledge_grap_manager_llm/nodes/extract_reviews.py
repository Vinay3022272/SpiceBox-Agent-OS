...
def _parse_review_rows(rows: list[dict], source_name: str = "db_review") -> list[ExtractedReview]:
    ...
    if rows:
        ...
        for key in sample.keys():
            ...
            if "slug" in key_lower:
                col_map["product_slug"] = key
            elif "product" in key_lower and "name" in key_lower:
                col_map["product_name"] = key
            elif "product" in key_lower and "slug" not in key_lower:
                col_map["product_name"] = key
            elif "name" in key_lower and "product" not in col_map and "customer" not in key_lower:
                col_map["product_name"] = key
                ...
                col_map["title"] = key
            elif any(k in key_lower for k in ["comment", "feedback", "body", "customer_text", "content", "text"]) or (key_lower == "review"):
                col_map["body"] = key
    ...
    for row in rows:
        product_slug = str(row.get(col_map.get("product_slug", "product_slug"), "")).strip()
        ...
        if not product_name or product_name.lower() in ("unknown", "n/a", "none"):
            product_name = str(row.get("name", row.get("product", product_slug or "Unknown"))).strip()
        if not product_slug and product_name:
...
def _synthesize_reviews(product_name: str, product_slug: str, reviews: list[ExtractedReview]) -> ReviewSynthesis:
    """Synthesize reviews into sentiment summary using fast-path for single reviews and LLM for multi-reviews."""
    ratings = [r["rating"] for r in reviews if r["rating"] > 0]
    avg_rating = round(sum(ratings) / len(ratings), 1) if ratings else 0.0

    # Fast-path for small review sets (1-2 reviews)
    if len(reviews) <= 2:
        summaries = []
        for r in reviews:
            title = (r.get("title") or "").strip()
            body = (r.get("body") or "").strip()
            rating = r.get("rating", avg_rating)
            if title and body and title != "None":
                summaries.append(f'"{title}: {body}" ({rating}/5)')
            elif body and body != "None":
                summaries.append(f'"{body}" ({rating}/5)')
            elif title and title != "None":
                summaries.append(f'"{title}" ({rating}/5)')
        summary_text = " | ".join(summaries) if summaries else f"Rated {avg_rating}/5 by verified buyers."
        pros = [r.get("title").strip() for r in reviews if r.get("title") and r.get("title").strip() and r.get("title") != "None"]
        return ReviewSynthesis(
            product_slug=product_slug,
            product_name=product_name,
            avg_rating=avg_rating,
            total_reviews=len(reviews),
            sentiment_summary=f"Average rating: {avg_rating}/5. {summary_text}",
            top_pros=pros if pros else ["Verified buyer approval"],
            top_cons=[],
            best_reviews=[{
                "title": r.get("title", "Review") or "Review",
                "reviewer": r.get("reviewer", "Anonymous"),
                "rating": r.get("rating", avg_rating),
                "excerpt": (r.get("body") or r.get("title") or "")[:150],
            } for r in reviews],
        )
...
def extract_reviews(state: WikiState) -> dict:
    ...
    # 1. Dedicated review sources (deduplicated)
    review_sources = []
    for s in (classified.get("review", []) + classified.get("reviews", [])):
        if s not in review_sources:
            review_sources.append(s)
